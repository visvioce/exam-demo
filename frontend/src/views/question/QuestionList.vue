...
function normalizeTrueFalseAnswer(answer: unknown): string {
  if (answer == null || answer === '') return ''
  const s = String(answer).trim().toLowerCase()
  if (s === '正确' || s === 'true' || s === 'a') return '正确'
  if (s === '错误' || s === 'false' || s === 'b') return '错误'
  return String(answer)
}

function handleEdit(row: Question) {
  isEdit.value = true
  let correctAnswer: string | string[] = row.correctAnswer || ''
  // 判断题答案规范化：将 A/B/true/false 等统一转成 "正确"/"错误"
  if (row.type === 'TRUE_FALSE' && typeof correctAnswer === 'string') {
...
async function handleGenerate() {
  ...
  await aiFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      // 使用流式API，并保存取消函数
      // 实时展示打字机效果，流结束后通过 onComplete 回调解析 JSON 展示题目
      let fullContent = ''
          ...
          try {
            ...
            if (jsonMatch) {
              ...
              if (parsed.questions && Array.isArray(parsed.questions)) {
                ...
                ElMessage.success(`AI已生成 ${parsed.questions.length} 道题目`)
              } else {
                ElMessage.warning('AI返回格式不正确，未找到题目数据')
              }
            } else {
              ElMessage.warning('AI返回内容无法解析')
            }
          } catch (e) {
            ElMessage.warning('AI返回内容解析失败，请检查AI输出格式')
          }
        },
        onError: (error) => {
          generating.value = false
...
async function handleSaveAllQuestions() {
  ...
  for (let i = 0; i < generatedQuestions.value.length; i++) {
    ...
    try {
      ...
      // 仅简答题保存评分标准
      if (q.type === 'ESSAY' && q.scoringCriteria && q.scoringCriteria.length > 0) {
