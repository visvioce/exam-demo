...
public class AiConfigController {
    ...
    @Operation(summary = "获取我的AI配置列表", description = "获取当前用户的所有AI API配置")
    @GetMapping("/my")
    ...
    @Operation(summary = "获取我的活跃配置", description = "获取当前用户当前激活的AI配置")
    ...
    @Operation(summary = "获取配置详情", description = "根据ID获取AI配置详细信息")
    @GetMapping("/{id}")
    ...
    @Operation(summary = "创建AI配置", description = "新增一个AI API配置，自动验证并设置默认模型")
    @PostMapping
    public Result<AiConfigResponse> create(@Valid @RequestBody AiConfigSaveRequest aiConfigSaveRequest, HttpServletRequest request) {
        ...
        String activeModel = aiConfigSaveRequest.getActiveModel();
        ...
        } else if (activeModel != null && !activeModel.isEmpty()) {
            aiConfig.setActiveModel(activeModel);
        }
    ...
    @Operation(summary = "更新AI配置", description = "更新指定AI配置信息，API Key未修改时保留原值")
    @PutMapping("/{id}")
    public Result<Boolean> update(@PathVariable Long id, @Valid @RequestBody AiConfigSaveRequest aiConfigSaveRequest, HttpServletRequest request) {
        ...
        aiConfig.setApiKey(aiConfigSaveRequest.getApiKey());
        aiConfig.setModels(aiConfigSaveRequest.getModels());
        aiConfig.setActiveModel(aiConfigSaveRequest.getActiveModel());
    ...
    @Operation(summary = "删除AI配置", description = "根据ID删除指定AI配置")
    @DeleteMapping("/{id}")
    ...
    @Operation(summary = "激活模型", description = "将指定模型设为当前使用的模型")
